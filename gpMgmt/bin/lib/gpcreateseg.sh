...
CREATE_QES_MIRROR () {
    ...
    if [ $HBA_HOSTNAMES -eq 0 ];then
        local MIRROR_ADDRESSES=($("$GPHOME"/libexec/ifaddrs --no-loopback))
        for ADDRESS in "${MIRROR_ADDRESSES[@]}"
        do
            CIDR_ADDR=$(GET_CIDRADDR $ADDRESS)
            PG_HBA_ENTRIES="${PG_HBA_ENTRIES}\nhost  replication ${GP_USER} ${CIDR_ADDRESS} trust"
        done
    else
        PG_HBA_ENTRIES="\nhost  replication ${GP_USER} ${GP_HOSTADDRESS} trust"
    fi
    RUN_COMMAND_REMOTE ${PRIMARY_HOSTADDRESS} "${EXPORT_GPHOME}; . ${GPHOME}/greenplum_path.sh; echo ${PG_HBA_ENTRIES} >> ${PRIMARY_DIR}/pg_hba.conf; pg_ctl -D ${PRIMARY_DIR} reload"
