...
Feature: gprecoverseg tests
    ...
    Scenario: When force recovering and the Pid corresponds to a non postgres process
        Given the database is running
    ...
    Scenario: When using pg_rewind to recover and the Pid corresponds to a non postgres process
        Given the database is running
        And all the segments are running
        And the segments are synchronized
        And the "primary" segment information is saved
